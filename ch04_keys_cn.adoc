...
Alice想要支付给Bob，但将验证她交易的数千个比特币全节点不知道Alice或Bob是谁——而我们希望保持这种状态以保护他们的隐私。
Alice需要传达的是，Bob应该收到她的一些比特币，而不会将交易的任何方面与Bob的真实身份或Bob收到的其他比特币支付联系在一起。
Alice使用的方法必须确保只有Bob能够进一步花费他所收到的比特币


The original Bitcoin paper describes a very simple scheme for achieving
those goals, shown in <<pay-to-pure-pubkey>>.  

原始的比特币论文描述了一种实现这些目标的非常简单的方案，显示在<<pay-to-pure-pubkey>>中
...
像Bob这样的接收者接受一笔比特币交易到一个公钥，该交易由支付者（比如Alice）签名。
Alice要花费的比特币之前已经收到到她的一个公钥，她使用相应的私钥生成她的签名。
全节点可以验证Alice的签名是对哈希函数的输出进行了承诺，而这个哈希函数本身又对Bob的公钥和其他交易细节进行了承诺
...
在本章中，我们将研究公钥、私钥、签名和哈希函数，然后将它们全部结合在一起，描述现代比特币软件使用的地址。
...
自公钥密码学的发明以来，已经发现了几种适合的数学函数，例如素数指数运算和椭圆曲线乘法。
这些数学函数在一方向上易于计算，在当前计算机和算法下，反向计算几乎是不可行的。
基于这些数学函数，密码学可以实现不可伪造的数字签名。比特币使用椭圆曲线加法和乘法作为其密码学的基础。
...
在比特币中，我们可以使用公钥密码学创建一个密钥对来控制对比特币的访问。
密钥对包括一个私钥和从私钥派生的一个公钥。公钥用于接收资金，而私钥用于签署交易以花费资金。
...
公钥和私钥之间存在数学关系，允许使用私钥对消息生成签名。这些签名可以根据公钥进行验证，而无需暴露私钥。
...
在某些钱包实现中，私钥和公钥被一起存储为一个密钥对，以方便使用。然而，公钥可以从私钥计算得到，所以也有可能只存储私钥。
====
...
比特币钱包包含一组密钥对，每个密钥对由一个私钥和一个公钥组成。私钥（_k_）是一个数字，通常是从随机选择的数字派生而来的。
从私钥开始，我们使用椭圆曲线乘法，一个单向加密函数，生成一个公钥（_K_）。
...
非对称加密并不用于对交易进行“加密”（使其保密）。相反，非对称加密的一个有用特性是能够生成数字签名。
私钥可以应用于交易以生成一个数字签名。这个签名只能由知晓私钥的人产生。
然而，任何人只要有公钥和交易的访问权限，都可以使用它们来验证签名。
非对称加密的这个有用特性使得任何人都能够验证每笔交易上的每个签名，同时确保只有私钥的所有者能够产生有效的签名。
****
...
私钥只是一个随机选择的数字。对私钥的控制是用户对与相应的比特币公钥相关的所有资金的控制的根源。
私钥用于创建用于花费比特币的签名，以证明对交易中使用的资金的控制权。
私钥必须始终保持秘密，因为向第三方透露它等同于将对该密钥保护的比特币的控制权交给他们。
私钥还必须备份和保护免受意外丢失，因为如果丢失，它将无法恢复，由它保护的资金也将永远丢失。
...
[TIP]
====
比特币的私钥只是一个数字。你可以使用一枚硬币、铅笔和纸张随机选择你的私钥：将硬币抛掷256次，
你就得到了一个可以在比特币钱包中使用的随机私钥的二进制数字。
然后可以从私钥生成公钥。不过要小心，因为任何不完全随机的过程都可能会显著降低你的私钥和所控制的比特币的安全性。
====
...
生成密钥的第一步，也是最重要的一步，是找到一个安全的随机来源（计算机科学家称之为“熵”）。
创建比特币密钥几乎就像是“在1到2^256^之间选择一个数字。
”你用来选择这个数字的确切方法并不重要，只要它不可预测也不可重复。比特币软件使用密码学安全的随机数生成器来生成256位的熵。
